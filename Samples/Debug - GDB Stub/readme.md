...
- **Windows:** `getcompiler.ps1` downloads gdb-multiarch 15.1 from https://static.grumpycoder.net/pixel/gdb-multiarch-windows/ and installs it to `tools/bin/win/gdb-multiarch.exe`, which is where `launch.json` looks for it.
...
- `launch.json` connects to the remote GDB stub on `localhost:1234` using `gdb-multiarch`. `launch.json` has per-OS `miDebuggerPath` entries (Windows uses `tools/bin/win/gdb-multiarch.exe`; Linux uses `gdb-multiarch` from `PATH`; macOS uses `/opt/homebrew/bin/gdb-multiarch`, since VS Code launched from the Dock does not inherit the shell `PATH`). The `tasks.json` tasks also have Linux, Windows and macOS variants.
